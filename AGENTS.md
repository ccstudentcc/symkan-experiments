...
- When `docs/symkan_manuscript.md` is revised for paper or defense use, keep presentation sources under `docs/slides/` and sync their entry points with `docs/index.md`, `docs/project_map.md`, and `README.md`.
- Treat `docs/slides/*.aux`, `*.log`, `*.nav`, `*.snm`, `*.toc`, `*.fls`, `*.fdb_latexmk`, and the compiled deck PDF as local build artifacts; keep Beamer source files in git, not the generated compile outputs.
...
- When adding, removing, or re-scoping a core document, also sync `docs/documentation_governance.md`, `docs/index.md`, `docs/project_map.md`, `README.md`, `CONTRIBUTING.md`, and `docs/engineering_release_checklist.md`.

...
| V1 | w/o Stagewise Train | off | on | on | on | 一次性训练替代 stagewise |
| V2 | w/o Progressive Pruning | on | off | on | on | `max_prune_rounds=0` |
| V3 | w/o Input Compaction | on | on | off | on | 需新增开关 |
| V4 | w/o Layerwise Finetune | on | on | on | off | `layerwise_finetune_steps=0` |
...
V0,42,120,85,0.8123,14.20,0.9412,0.8821,126.4,218.9,11,962
```
...
| --- | --- | --- | --- | --- | --- | --- |
| V0 | 0.81 ± 0.01 | 14.2 ± 1.3 | 0.94 ± 0.01 | 0.88 ± 0.02 | 126.4 ± 9.7 | 218.9 ± 12.4 |
| V1 | ... | ... | ... | ... | ... | ... |
...
1. `ablation/configs.py`
   - 定义 V0~V4 的参数覆写规则。
2. `ablation/runner.py`
   - 读取 base args。
   - 按 variant 覆写后执行单 run。
3. `ablation/collect.py`
   - 聚合 15 次运行结果。
   - 产出明细表与汇总表。
4. `ablation/report.py`
   - 生成 markdown 结论草稿（可选）。

### 9.3 关键 CLI 建议

```text
--ablation-variants full,wostagewise,wopruning,wocompact,wolayerwiseft
--stagewise-seeds 42,52,62
...
1. `symbolize_pipeline` 已新增 `enable_input_compaction` 参数（默认 True）。
2. `symkanbenchmark.py` 已透传该参数。
3. 训练阶段已补充计时：
   - 总时长 `stage_total_seconds`。
   - 每阶段时长 `stage_logs.stage_seconds`（并含 `train_seconds/prune_seconds`）。

这样可确保 B2 与“各阶段训练时间”两项要求可被严格度量。
...
6. `benchmark_ablation/ablation_conclusion.md`

以上 6 项齐备后，才能认为消融实验可复审、可复现实证结论。

...
基于 `benchmark_ablation/` 与 `benchmark_ab/comparison/` 的结果，当前设计决策已经从“可行”收敛到“可默认”：
...
这组结论的核心不是“某个技巧更聪明”，而是把每个模块的职责钉死：
...
3. A/B 结论用于解释“为什么这么配”：
    - 最新重跑结果下，`baseline` 在 `final_acc` 与 `macro_auc` 上均领先 `adaptive`/`adaptive_auto`。
    - `adaptive` 系列的主要价值仍是流程控制与部分耗时场景的工程可调性，不应表述为“默认精度增强”。
    - 因此默认策略强调兼容、可复现和明确开关语义，而不是宣传“必然提精度”。

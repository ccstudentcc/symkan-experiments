...
**结论：Stagewise 训练是可用性前提，去掉后并非“退化一点”，而是整条链路失效。**
...
时间确实变短（`stage_total_seconds` -66.07%，`symbolic_total_seconds` -50.46%），但这是因为跳过了必要的稀疏化准备阶段，最终输出质量不可用。这个结果直接支持“先可控稀疏，再符号化”的架构假设。

### 3.2 V2：去掉 Progressive Pruning

**结论：去掉渐进剪枝可以换一点分类指标，但代价是模型与公式体积膨胀。**
...
`validation_mean_r2` 从 -0.6135 提升到 -0.4976（绝对 +0.1159），说明保留更多边可改善数值拟合，但仍在负值区间。该模块的核心价值仍是复杂度治理，不是提精度。

### 3.3 V3：去掉 Input Compaction

**结论：输入压缩是“公式数值拟合质量”与“符号化成本”之间的硬权衡。**
...
保留全部输入维度后，公式数值重拟合质量明显提升，但符号化开销也同步放大。若目标是吞吐与稳定复现，保留输入压缩更实用；若目标是追求更高的公式数值一致性，可考虑关闭。

### 3.4 V4：去掉 LayerwiseFT

**结论：在当前 2 层 KAN 设置下，关闭 LayerwiseFT 维持同等分类表现，并显著降低符号化耗时。**
...
最新重跑中 `full` 与 `w/o LayerwiseFT` 的精度方差同量级（0.0013 vs 0.0014），因此不能再使用“方差显著缩小”作为核心证据；但“几乎零精度收益 + 明显时间成本”仍然成立。
...
在 2 层 KAN（`depth=2`）里，LayerwiseFT 只在 `l < depth - 1` 条件下触发一次，补偿窗口天然有限。`fix_symbolic` 冻结函数族后，LayerwiseFT 本质是受限模型的局部修补，容易出现“前层固定分布 -> 后层局部适配 -> 下一步符号化继承偏差”的序列偏差。
...
这组结果与理论一致：在 2 层设置里，LayerwiseFT 不是决定性增益项，更像高成本的可选微调。
...
- 原始汇总：`benchmark_ablation/ablation_runs_summary.csv`
- 原始明细：`benchmark_ablation/ablation_runs_raw.csv`
- LayerwiseFT 改进对比：`benchmark_ablation/layerwiseft_improved_analysis/{comparison_summary.csv,delta_new_vs_full.csv,delta_new_vs_wolayerwiseft.csv}`
- 各变体运行目录：`benchmark_ablation/{full,wostagewise,wopruning,wocompact,wolayerwiseft}/`

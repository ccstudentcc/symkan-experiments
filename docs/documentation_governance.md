...
2. `docs/symkan_usage.md`、`docs/kan_parameters.md`：库层与 notebook 参数说明。
3. `docs/symkanbenchmark_usage.md`、`docs/ablation_usage.md`：脚本执行语义、参数与输出约定。
4. `docs/full_experiment_runbook.md`：完整复跑步骤、检查项与产物核对。
...
3. 执行文档写“如何运行、检查什么产物、哪些参数属于哪一层”，不承担研究结论归纳。
4. 报告文档写“本轮实验看到了什么、证据边界在哪里”，不把补充切片写成正式 paired 证据。
5. 发布文档写“发布前必须检查什么”，不重写方法说明或报告正文。
6. 任务跟踪文档写“当前在做什么、做到哪一步、还有哪些风险”，不承担方法说明或发布记录职责。
...
5. 检查是否需要同步 `AGENTS.md`，以减少未来重复摸索。
6. 若为复杂任务，检查 `SPEC.md`、`IMPLEMENTATION_PLAN.md`、`TASK_STATUS.md` 是否仍反映当前目标。

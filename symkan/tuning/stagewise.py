...
def _create_validation_split(dataset, ratio: float, seed: Optional[int], min_val_samples: int = 10):
    """@brief 在现有 dataset 上原地风格创建验证集切分。"""
...
def stagewise_train(
    ...
    @param use_validation 是否启用验证集引导剪枝；默认关闭以保持旧行为。
    @param validation_ratio 训练集切出验证集比例；已有验证集时忽略。
    @param validation_seed 验证集切分随机种子。
    @param validation_min_samples 验证集最少样本数。
    @param adaptive_threshold 是否启用自适应剪枝阈值。
    @param threshold_base_step 自适应阈值基础步长。
    @param threshold_min 自适应阈值下界。
    @param threshold_max 自适应阈值上界。
    @param success_boost 连续成功时的阈值增益系数。
    @param failure_penalty 连续失败时的阈值惩罚系数。
    @param min_gain_threshold 最近剪枝平均收益低于该值时提前停止。
    @param max_prune_attempts 单阶段最多剪枝尝试次数。
    @param adaptive_lamb 是否根据稀疏进度调整 lamb。
    @param min_lamb_ratio lamb 下界倍数。
